...
- **fnOS 文件选择器**：输出目录可用 fnOS 自带文件管理器浏览选择（需 fnOS 1.2.0401+，
  不支持时回退手动输入）。
- **应用与引擎更新**：Web 界面「应用与引擎更新」卡片可检查应用新版本（本项目
  Releases）与核心引擎新版本（上游 N_m3u8DL-RE Releases）；引擎有新版本时可在
  **应用内一键升级**（后台下载→解压→`--version` 校验→原子替换，不影响已有任务），
  应用有新版本时一键把 fpk 下载到共享目录，到应用中心手动安装即可；GitHub 不可达
  （内网/受限网络）时优雅降级，仅显示本地版本并提示网络原因。
